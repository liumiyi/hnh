...
Motor_Info motor_info2;//两个摩擦轮电机信息的结构体
...
void CAN_Transmit_1(int16_t v)//控制右摩擦轮电机，id是2
{
...
void CAN_Transmit_2(int16_t v)//控制左摩擦轮电机，id是3
{
  TxMessage.IDE=CAN_ID_STD;
  TxMessage.RTR=CAN_RTR_DATA;
  TxMessage.DLC=8;
...
void CAN_Transmit_3(int16_t v)//控制拨盘电机，id是1
{
  TxMessage.IDE=CAN_ID_STD;
  TxMessage.RTR=CAN_RTR_DATA;
  TxMessage.DLC=8;
...
void update_angle(Motor_Info* motor_info)//单环变多环，给绝对式编码器使用
{
...
//void dial_angle(Motor_Info* motor_info)//拨盘电机角度和速度编码器值解算
//{
//    if(motor_info->rotor_angle - motor_info->last_ecd > 4096) 
//      motor_info->round_cnt --;
//    else if(motor_info->rotor_angle - motor_info->last_ecd < -4096)
